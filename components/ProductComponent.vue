    ...
    <NuxtLink :to="`/item/${product.id}`">
      ...
      <div id="productDetails">
        <span class="flex items-center justify-start gap-3 px-1 pt-1">
          ...
          <span class="text-gray-500 text-sm text-light line-trough"
            >${{ oldPriceComputed }}</span
          >
        </span>
        <span
          class="px-1 relative -top-1.5 text-[#ff6674] text-xs text-semibold"
          >Extra 5% off</span
        >
        <div class="flex items-center gap-1 px-1 relative -top-1">
          <span
            class="bg-[#fd374f] text-white text-[9px] font-semi-bold px-1.5 rounded-sm"
            >Welcome Deal
          </span>
          <span
        ...
        <p class="flex itens-center px-1 pt-0.5 text-xs text-[#252525]">
          5,000+sold
          <Icon
